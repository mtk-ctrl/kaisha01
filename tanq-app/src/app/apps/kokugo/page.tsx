...
const STORAGE_KEY = 'tanq_kokugo_v1'

interface KokugoSave {
  levelStars: Record<number, 0 | 1 | 2 | 3>
}

function loadSave(): KokugoSave {
...
function writeSave(save: KokugoSave) {
...
interface QuizState {
  ...
  confirmed: boolean
  wrong: number
  answers: { correct: boolean; q: KokugoQuestion }[]
}
...
export default function KokugoPage() {
  ...
  const next = useCallback(() => {
    setQuiz(q => {
      if (!q) return q
      const nextIdx = q.current + 1
      if (nextIdx >= q.questions.length) {
        // 終了 → resultへ
        return { ...q, current: nextIdx, confirmed: false, selected: null }
      }
      return { ...q, current: nextIdx, confirmed: false, selected: null }
    })
  }, [])
  ...
  if (view === 'map') {
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #fdf4ff 0%, #eff6ff 100%)', padding: '16px' }}>
        {/* header */}
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '24px' }}>
            ...
            <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: '#1e1b4b', margin: '0 0 4px' }}>国語クイズ</h1>
            <p style={{ color: '#6b7280', fontSize: '14px', margin: 0 }}>慣用句・四字熟語 全20レベル</p>
          </div>
            ...
            {KOKUGO_LEVEL_META.map(meta => {
              ...
              return (
                <button
                  ...
                  disabled={locked}
                  style={{
                    background: locked ? '#f3f4f6' : cleared ? 'white' : 'white',
                ...
                >
                  ...
                  <div style={{ fontSize: '11px', color: '#9ca3af', marginTop: '4px' }}>
                    {getQuestionsForLevel(meta.level).length}問
                  </div>
                </button>
              )
            })}
  ...
  if (view === 'quiz' && quiz && quiz.current < quiz.questions.length) {
    ...
    const meta = KOKUGO_LEVEL_META[quiz.level - 1]
    const total = quiz.questions.length
    const progress = quiz.current / total

    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #fdf4ff 0%, #eff6ff 100%)', padding: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div style={{ width: '100%', maxWidth: '520px' }}>
          ...
          {/* progress bar */}
          <div style={{ background: '#e5e7eb', borderRadius: '999px', height: '6px', marginBottom: '20px', overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${progress * 100}%`, background: 'linear-gradient(90deg, #8b5cf6, #ec4899)', borderRadius: '999px', transition: 'width 0.3s' }} />
          </div>

          {/* question card */}
          <div style={{ background: 'white', borderRadius: '20px', padding: '24px', marginBottom: '16px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
            <div style={{ fontSize: '11px', color: '#8b5cf6', fontWeight: '600', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              {q.type === 'kanyo' ? '慣用句' : '四字熟語'}
            </div>
            <p style={{ fontSize: '19px', fontWeight: 'bold', color: '#1e1b4b', margin: 0, lineHeight: 1.5 }}>{q.q}</p>
          </div>
          ...
          {!quiz.confirmed ? (
            <button
              ...
              style={{ width: '100%', background: quiz.selected === null ? '#e5e7eb' : 'linear-gradient(135deg, #8b5cf6, #ec4899)', color: quiz.selected === null ? '#9ca3af' : 'white', border: 'none', borderRadius: '14px', padding: '16px', fontSize: '16px', fontWeight: 'bold', cursor: quiz.selected === null ? 'not-allowed' : 'pointer', transition: 'all 0.15s' }}
            >
              こたえる
            </button>
          ) : (
            <button
              onClick={next}
              style={{ width: '100%', background: 'linear-gradient(135deg, #8b5cf6, #ec4899)', color: 'white', border: 'none', borderRadius: '14px', padding: '16px', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer' }}
            >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            {passed && quiz.level < 20 && (
              <button
                ...
                style={{ width: '100%', background: 'linear-gradient(135deg, #8b5cf6, #ec4899)', color: 'white', border: 'none', borderRadius: '14px', padding: '14px', fontSize: '15px', fontWeight: '600', cursor: 'pointer' }}
              >
                次のレベルへ Lv {quiz.level + 1} →
              </button>
            )}
            {quiz.level === 20 && passed && (
              <div style={{ textAlign: 'center', color: '#7c3aed', fontWeight: 'bold', fontSize: '16px', padding: '8px' }}>🏆 全レベルクリア！国語マスター！</div>
            )}
            <button
              onClick={() => setView('map')}
