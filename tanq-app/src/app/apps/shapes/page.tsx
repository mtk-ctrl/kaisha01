...
const SHAPES: Shape[] = [
  ...
  },
  {
    ...
    color: '#60a5fa',
    description: 'よこもたてもおなじ長さ',
  },
  {
    name: 'rectangle',
    japanese: 'ちょうほうけい（長方形）',
    corners: 4, sides: 4,
  ...
  },
  {
    ...
    japanese: 'ごかくけい（五角形）',
    corners: 5, sides: 5,
    svgPath: 'M50,8 L92,38 L75,85 L25,85 L8,38 Z',
    color: '#f0c040',
  ...
  },
  {
    ...
    japanese: 'ろっかくけい（六角形）',
    corners: 6, sides: 6,
    svgPath: 'M50,5 L90,27 L90,73 L50,95 L10,73 L10,27 Z',
    color: '#c4a8ff',
...
function makeQuestion(shape: Shape): ShapeQuestion {
  ...
  if (type === 'corners') {
    question = 'かどは何個？'
    correct = `${shape.corners}個`
    const opts = shuffle([0, 3, 4, 5, 6, 8, 10].filter((n) => n !== shape.corners)).slice(0, 3).map((n) => `${n}個`)
    return {
      type, shape, question, correct,
  ...
  question = 'へんは何本？'
  correct = `${shape.sides}本`
  const opts = shuffle([1, 3, 4, 5, 6, 8, 10].filter((n) => n !== shape.sides)).slice(0, 3).map((n) => `${n}本`)
  return {
    type, shape, question, correct,
    choices: shuffle([correct, ...opts]),
    svgPath: shape.svgPath,
...
export default function ShapesQuiz() {
  ...
  const startGame = useCallback(() => {
    const shapes = shuffle(SHAPES).slice(0, TOTAL)
    const q = shapes.map(makeQuestion)
    setQueue(q)
    setIndex(0)
    setScore(0)
    setMiss(0)
    ...
    return (
      <div className="min-h-screen bg-[#0d2248] text-[#e8f0fe] font-sans flex flex-col items-center justify-center px-6 text-center">
        ...
        <p className="text-[#94a3c4] mb-6 max-w-xs leading-relaxed">
          図形を見てなまえや数を答えよう！<br />三角形から星形まで全10問！
        </p>
        <button
          onClick={startGame}
          className="px-12 py-5 rounded-2xl font-black text-xl text-[#050b14] transition-all hover:scale-[1.04]"
          style={{ background: '#c4a8ff', boxShadow: '0 0 40px rgba(196,168,255,0.4)' }}
        >
          スタート！
        </button>
      </div>
  ...
  if (phase === 'result') {
    const rank = score >= 9 ? '🏆 図形マスター！' : score >= 7 ? '🥇 すごい！' : score >= 5 ? '🥈 よくできた' : '🥉 もう一回！'
    return (
      <div className="min-h-screen bg-[#0d2248] text-[#e8f0fe] font-sans flex flex-col items-center justify-center px-6 text-center">
  ...
  return (
    <div className="min-h-screen bg-[#0d2248] text-[#e8f0fe] font-sans flex flex-col items-center justify-center px-6">
      ...
      <div className="text-center">
        <p className="text-[#94a3c4] text-sm mb-6 tracking-widest uppercase">{q.question}</p>
        <div className="flex justify-center mb-6">
          <ShapeSVG shape={q.shape} size={120} />
        </div>
        <p className="text-[#94a3c4] text-xs mb-8">{q.shape.description}</p>
        ...
        <div className="grid grid-cols-2 gap-4 max-w-sm mx-auto">
          {q.choices.map((c) => {
            ...
            let text = '#e8f0fe'
            if (selected !== null) {
              if (isCorrect) { bg = 'rgba(196,168,255,0.25)'; border = '#c4a8ff'; text = '#c4a8ff' }
