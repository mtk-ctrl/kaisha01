...
// - 地形と気候のほかの地理単元（農業・工業など）は単元マップで「近日公開」として全体像のみ見せる
...
const STORAGE_KEY = 'tanq_chiri_v1'
const MAX_LEVEL = CHIRI_LEVEL_META.length

interface ChiriSave {
  levelStars: Record<number, 0 | 1 | 2 | 3>
}

function loadSave(): ChiriSave {
...
function writeSave(save: ChiriSave) {
...
function prepare(q: ChiriQuestion): PreparedQuestion {
  if (q.type === 'choice') {
...
interface PlayState {
  ...
  wrong: number
  answers: { correct: boolean; q: ChiriQuestion }[]
}
...
export default function ChiriPage() {
  const [view, setView] = useState<View>('map')
  const [save, setSave] = useState<ChiriSave>({ levelStars: {} })
  ...
  const persistSave = useCallback((next: ChiriSave) => {
    setSave(next)
    writeSave(next)
  }, [])
  ...
  const startLevel = useCallback((level: number) => {
    const questions = shuffle(getChiriQuestionsForLevel(level)).map(prepare)
  ...
  useEffect(() => {
    ...
    if (play.current >= play.questions.length) {
      ...
      saveScore('chiri', play.questions.length - play.wrong, play.questions.length, `Lv${play.level}`)
      setView('result')
    }
  ...
  if (view === 'map') {
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%)', padding: '16px' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          <Link href="/juken" style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', color: '#0369a1', textDecoration: 'none', fontSize: '14px', marginBottom: '12px' }}>
            ← 中学受験ハブにもどる
          </Link>
          ...
          <div style={{ background: 'white', borderRadius: '16px', padding: '14px 16px', marginBottom: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
            <div style={{ fontSize: '12px', fontWeight: '600', color: '#0369a1', marginBottom: '10px' }}>🗺️ 地理 単元マップ</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', alignItems: 'center' }}>
              {CHIRI_UNIT_MAP.map(unit => (
                <span key={unit.name} style={{
                  fontSize: '11px', fontWeight: '600', padding: '4px 9px', borderRadius: '999px',
                  background: unit.implemented ? '#e0f2fe' : '#f3f4f6',
                  color: unit.implemented ? '#0369a1' : '#9ca3af',
                  border: unit.implemented ? '1px solid #38bdf8' : '1px dashed #d1d5db',
                }}>
                  {unit.name}{unit.implemented ? '' : '（近日公開）'}
                </span>
              ))}
            </div>
            <p style={{ margin: '10px 0 0', fontSize: '11px', color: '#94a3b8' }}>農業・工業などは近日公開。まずは「地形と気候」をきわめよう！</p>
          </div>
          ...
          <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', justifyContent: 'center' }}>
            {[
              ...
              <div key={label} style={{ background: 'white', borderRadius: '12px', padding: '12px 20px', textAlign: 'center', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
                <div style={{ fontSize: '22px', fontWeight: 'bold', color: '#0284c7' }}>{value}<span style={{ fontSize: '13px', color: '#9ca3af' }}>{unit}</span></div>
            ...
            {CHIRI_LEVEL_META.map(meta => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontSize: '13px', fontWeight: '600', color: locked ? '#9ca3af' : '#0c4a6e', marginBottom: '2px' }}>{meta.title}</div>
          ...
          <div style={{ marginTop: '20px', background: 'white', borderRadius: '16px', padding: '16px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {[
                { emoji: '🧭', label: '地理の一歩', lv: 1 },
                { emoji: '🏞️', label: '川と平野マスター', lv: 3 },
                { emoji: '🌦️', label: '気候マスター', lv: 5 },
                { emoji: '🏆', label: '地形と気候マスター', lv: 6 },
              ].map(b => {
                const earned = (save.levelStars[b.lv] ?? 0) >= 1
                return (
                  <div key={b.lv} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: earned ? '#e0f2fe' : '#f3f4f6', borderRadius: '999px', padding: '4px 10px', opacity: earned ? 1 : 0.4 }}>
                    <span style={{ fontSize: '16px' }}>{b.emoji}</span>
                    <span style={{ fontSize: '11px', fontWeight: '600', color: earned ? '#0369a1' : '#9ca3af' }}>{b.label}</span>
                  </div>
                )
              })}
            </div>
  ...
  if (view === 'play' && play && play.current < play.questions.length) {
    const pq = play.questions[play.current]
    const q = pq.src
    const meta = CHIRI_LEVEL_META[play.level - 1]
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%)', padding: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div style={{ width: '100%', maxWidth: '520px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
            <button onClick={() => setView('map')} style={{ background: 'none', border: 'none', color: '#0369a1', cursor: 'pointer', fontSize: '14px' }}>← もどる</button>
          ...
          <div style={{ background: '#bae6fd', borderRadius: '999px', height: '6px', marginBottom: '20px', overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${progress * 100}%`, background: 'linear-gradient(90deg, #0ea5e9, #0369a1)', borderRadius: '999px', transition: 'width 0.3s' }} />
          </div>

          <div style={{ background: 'white', borderRadius: '20px', padding: '22px 24px', marginBottom: '16px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
            <div style={{ fontSize: '11px', color: '#0369a1', fontWeight: '600', marginBottom: '8px', letterSpacing: '0.05em' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '16px' }}>
              {pq.choices.map((choice, i) => {
                ...
                if (play.confirmed) {
                  ...
                  bg = '#e0f2fe'; border = '2px solid #0ea5e9'; color = '#0369a1'
                }
            ...
            <div style={{ marginBottom: '16px' }}>
                  ...
                  return (
                    ...
                    >
                      <span style={{
                        ...
                        background: pickedAt !== null ? '#0ea5e9' : '#f5f5f4',
              ...
              {!play.confirmed && play.picked.length > 0 && (
                <button onClick={() => setPlay(p => p ? { ...p, picked: [] } : p)}
                  style={{ marginTop: '8px', background: 'none', border: 'none', color: '#0369a1', fontSize: '12px', cursor: 'pointer', textDecoration: 'underline' }}>
                  ならべなおす
                </button>
              )}
            </div>
          ...
          {play.confirmed && (
            <div style={{ marginBottom: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {play.firstWrong === null && play.attempt === 0 ? (
                ...
                <div style={{ background: '#eff6ff', border: '1px solid #93c5fd', borderRadius: '14px', padding: '14px 16px' }}>
                  <div style={{ fontSize: '12px', color: '#0369a1', fontWeight: '600', marginBottom: '4px' }}>
                    {play.secondCorrect ? '✨ 再挑戦で正解！しくみをおさえよう' : '📖 なぜ？がわかる解説'}
                  </div>
                  {q.type === 'order' && (
                    <p style={{ margin: '0 0 8px', fontSize: '13px', color: '#0c4a6e', fontWeight: '600', lineHeight: 1.9 }}>
                      正しい順番: {q.items.map((it, i) => (
                        <span key={i}>{i > 0 && ' → '}<Furigana text={it} /></span>
                      ))}
                    </p>
                  )}
                  <p style={{ margin: 0, fontSize: '14px', color: '#1e3a5f', lineHeight: 1.8 }}><Furigana text={q.explain} /></p>
                  <p style={{ margin: '6px 0 0', fontSize: '13px', color: '#0369a1', fontWeight: '600' }}>まちがえてもだいじょうぶ。「なぜ」でおぼえれば、つぎはとける！</p>
                </div>
              )}
            </div>
          )}
          ...
          {!play.confirmed ? (
            <button
              onClick={confirm}
              disabled={!canConfirm}
              style={{ width: '100%', background: !canConfirm ? '#e7e5e4' : 'linear-gradient(135deg, #0ea5e9, #0369a1)', color: !canConfirm ? '#9ca3af' : 'white', border: 'none', borderRadius: '14px', padding: '16px', fontSize: '16px', fontWeight: 'bold', cursor: !canConfirm ? 'not-allowed' : 'pointer', transition: 'all 0.15s' }}
            >
            ...
            <button
              onClick={next}
              style={{ width: '100%', background: 'linear-gradient(135deg, #0ea5e9, #0369a1)', color: 'white', border: 'none', borderRadius: '14px', padding: '16px', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer' }}
            >
        ...
        <div style={{ width: '100%', maxWidth: '520px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <button
              ...
              style={{ width: '100%', background: 'white', border: '2px solid #0ea5e9', color: '#0369a1', borderRadius: '14px', padding: '14px', fontSize: '15px', fontWeight: '600', cursor: 'pointer' }}
            >
              もう一度 Lv {play.level}
            </button>
            {passed && play.level < MAX_LEVEL && (
              <button
                ...
                style={{ width: '100%', background: 'linear-gradient(135deg, #0ea5e9, #0369a1)', color: 'white', border: 'none', borderRadius: '14px', padding: '14px', fontSize: '15px', fontWeight: '600', cursor: 'pointer' }}
              >
                次のレベルへ Lv {play.level + 1} →
              </button>
            )}
            {play.level === MAX_LEVEL && passed && (
              <div style={{ textAlign: 'center', color: '#0369a1', fontWeight: 'bold', fontSize: '16px', padding: '8px' }}>🏆 地形と気候マスター！農業・工業の単元も近日公開、おたのしみに！</div>
            )}
            <button
              onClick={() => setView('map')}
