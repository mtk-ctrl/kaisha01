...
function shuffleChoices(q: KanyoQuestion): KanyoQuestion {
...
interface QuizState {
  ...
  wrong: number
  answers: { correct: boolean; q: KanyoQuestion }[]
}
...
export default function KanyoPage() {
  const [view, setView] = useState<View>('map')
  const [save, setSave] = useState<KanyoSave>({ levelStars: {} })
  ...
  const persistSave = useCallback((next: KanyoSave) => {
    setSave(next)
    writeSave(next)
  }, [])
  ...
  const startLevel = useCallback((level: number) => {
    const questions = getKanyoQuestionsForLevel(level)
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #fff7ed 0%, #fef3c7 100%)', padding: '16px' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          <Link href="/lab" style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', color: '#ea580c', textDecoration: 'none', fontSize: '14px', marginBottom: '12px' }}>
            ← ラボにもどる
          </Link>
          <div style={{ textAlign: 'center', marginBottom: '24px' }}>
            <div style={{ fontSize: '48px', marginBottom: '8px' }}>🗣️</div>
            <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: '#7c2d12', margin: '0 0 4px' }}>慣用句クイズ</h1>
          ...
          <div style={{ display: 'flex', gap: '12px', marginBottom: '24px', justifyContent: 'center' }}>
            {[
              ...
              <div key={label} style={{ background: 'white', borderRadius: '12px', padding: '12px 20px', textAlign: 'center', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
                <div style={{ fontSize: '22px', fontWeight: 'bold', color: '#ea580c' }}>{value}<span style={{ fontSize: '13px', color: '#9ca3af' }}>{unit}</span></div>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontSize: '11px', color: '#9ca3af', marginTop: '4px' }}>
                    {getKanyoQuestionsForLevel(meta.level).length}問
                  </div>
                </button>
              )
            })}
          ...
          <div style={{ marginTop: '24px', background: 'white', borderRadius: '16px', padding: '16px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {[
                ...
                { emoji: '🗣️', label: '5レベル制覇', lv: 5 },
                { emoji: '📗', label: '10レベル制覇', lv: 10 },
                { emoji: '🔥', label: '15レベル制覇', lv: 15 },
                { emoji: '🏆', label: '慣用句マスター', lv: 20 },
              ].map(b => {
  ...
  if (view === 'quiz' && quiz && quiz.current < quiz.questions.length) {
    ...
    const meta = KANYO_LEVEL_META[quiz.level - 1]
    const total = quiz.questions.length
    const progress = quiz.current / total

    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #fff7ed 0%, #fef3c7 100%)', padding: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div style={{ width: '100%', maxWidth: '520px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
            <button onClick={() => setView('map')} style={{ background: 'none', border: 'none', color: '#ea580c', cursor: 'pointer', fontSize: '14px' }}>← もどる</button>
          ...
          <div style={{ background: '#fed7aa', borderRadius: '999px', height: '6px', marginBottom: '20px', overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${progress * 100}%`, background: 'linear-gradient(90deg, #f97316, #ef4444)', borderRadius: '999px', transition: 'width 0.3s' }} />
          </div>

          <div style={{ background: 'white', borderRadius: '20px', padding: '24px', marginBottom: '16px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
            <div style={{ fontSize: '11px', color: '#ea580c', fontWeight: '600', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              慣用句
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '16px' }}>
            {q.choices.map((choice, i) => {
              ...
              if (quiz.confirmed) {
                ...
                bg = '#ffedd5'; border = '2px solid #f97316'; color = '#c2410c'
              }
          ...
          {quiz.confirmed && (
            <div style={{ background: '#fff7ed', border: '1px solid #fdba74', borderRadius: '14px', padding: '14px 16px', marginBottom: '16px' }}>
              <div style={{ fontSize: '12px', color: '#ea580c', fontWeight: '600', marginBottom: '4px' }}>{quiz.firstWrong !== null ? '解説（おぼえちゃおう！）' : '解説'}</div>
              <p style={{ margin: 0, fontSize: '14px', color: '#7c2d12', lineHeight: 1.6 }}>{q.explain}</p>
              {quiz.firstWrong !== null && (
                <p style={{ margin: '6px 0 0', fontSize: '13px', color: '#ea580c', fontWeight: '600' }}>まちがえてもだいじょうぶ。つぎに出たら、もうとけるね！</p>
              )}
            </div>
          )}
          ...
          {!quiz.confirmed ? (
            <button
              ...
              style={{ width: '100%', background: quiz.selected === null ? '#e5e7eb' : 'linear-gradient(135deg, #f97316, #ef4444)', color: quiz.selected === null ? '#9ca3af' : 'white', border: 'none', borderRadius: '14px', padding: '16px', fontSize: '16px', fontWeight: 'bold', cursor: quiz.selected === null ? 'not-allowed' : 'pointer', transition: 'all 0.15s' }}
            >
            ...
            <button
              onClick={next}
              style={{ width: '100%', background: 'linear-gradient(135deg, #f97316, #ef4444)', color: 'white', border: 'none', borderRadius: '14px', padding: '16px', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer' }}
            >
        ...
        <div style={{ width: '100%', maxWidth: '520px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <button
              ...
              style={{ width: '100%', background: 'white', border: '2px solid #f97316', color: '#ea580c', borderRadius: '14px', padding: '14px', fontSize: '15px', fontWeight: '600', cursor: 'pointer' }}
            >
              もう一度 Lv {quiz.level}
            </button>
            {passed && quiz.level < 20 && (
              <button
                ...
                style={{ width: '100%', background: 'linear-gradient(135deg, #f97316, #ef4444)', color: 'white', border: 'none', borderRadius: '14px', padding: '14px', fontSize: '15px', fontWeight: '600', cursor: 'pointer' }}
              >
                次のレベルへ Lv {quiz.level + 1} →
              </button>
            )}
            {quiz.level === 20 && passed && (
              <div style={{ textAlign: 'center', color: '#ea580c', fontWeight: 'bold', fontSize: '16px', padding: '8px' }}>🏆 全レベルクリア！慣用句マスター！</div>
            )}
            <button
              onClick={() => setView('map')}
