'use client'

import React, { useState, useEffect, useRef, useMemo } from 'react'
import Link from 'next/link'
import { useParams, useRouter } from 'next/navigation'
import { YOUJI_QUESTIONS, YOUJI_QUESTIONS_PER_LEVEL, YOUJI_UNLOCK_THRESHOLD, YOUJI_TOTAL_LEVELS } from '@/data/thinkingYoujiData'
import { YOUJI_BADGES, YOUJI_TYPE_TO_BADGE } from '@/data/thinkingYoujiBadges'
import {
  loadYoujiProgress, saveYoujiProgress, isYoujiLevelUnlocked,
  getYoujiLevelProgress, getYoujiLevelStars, recordYoujiLevelResult, updateYoujiStreak,
  type YoujiProgress
} from '@/lib/thinkingYoujiProgress'

const SESSION_KEY = 'tanq-lab-auth'
const GUEST_FREE_LEVELS = 2

function getUserType(): 'guest' | 'tester' | 'member' {
  if (typeof window === 'undefined') return 'guest'
  const v = localStorage.getItem(SESSION_KEY)
  if (v === 'member') return 'member'
  if (v === 'tester') return 'tester'
  return 'guest'
}

// ─── うさぎキャラクター ────────────────────────────────────
type RabbitMood = 'normal' | 'happy' | 'excited' | 'dancing' | 'cheer' | 'celebrate'

function RabbitCharacter({ mood }: { mood: RabbitMood }) {
  const faces: Record<RabbitMood, string> = {
    normal:    '(｡•ᴗ•｡)',
    happy:     '(≧▽≦)♪',
    excited:   '(°▽°)✨',
    dancing:   '～(≧▽≦)ノ☆',
    cheer:     '(｡•ᴗ•｡)ᕙ',
    celebrate: '(≧◡≦)✨🎉',
  }
  const colors: Record<RabbitMood, string> = {
    normal:    'text-pink-500',
    happy:     'text-orange-500',
    excited:   'text-pink-600',
    dancing:   'text-purple-500',
    cheer:     'text-pink-400',
    celebrate: 'text-yellow-500',
  }
  return (
    <div className={`text-center font-mono text-sm font-bold transition-all duration-300 ${colors[mood]}
      ${mood === 'dancing' ? 'animate-bounce' : ''}
      ${mood === 'celebrate' ? 'animate-pulse' : ''}
    `}>
      {faces[mood]}
    </div>
  )
}

// ─── 花火 ────────────────────────────────────────────────
const EMOJIS = ['🎆', '🎇', '✨', '🎉', '⭐', '💫', '🌟', '🎊']

function Fireworks({ active }: { active: boolean }) {
  const particles = useMemo(() =>
    [...Array(14)].map((_, i) => ({
      top:      `${5 + Math.random() * 60}%`,
      left:     `${5 + Math.random() * 90}%`,
      duration: `${0.4 + Math.random() * 0.5}s`,
      delay:    `${Math.random() * 0.3}s`,
      emoji:    EMOJIS[i % EMOJIS.length],
    })),
  [])

  if (!active) return null
  return (
    <div className="fixed inset-0 pointer-events-none z-50 overflow-hidden">
      {particles.map((p, i) => (
        <div
          key={i}
          className="absolute text-2xl animate-bounce"
          style={{ top: p.top, left: p.left, animationDuration: p.duration, animationDelay: p.delay }}
        >
          {p.emoji}
        </div>
      ))}
    </div>
  )
}

// ─── バッジ取得通知 ───────────────────────────────────────
function BadgeToast({ badgeId, type, onDone }: { badgeId: string; type: 'silver' | 'gold'; onDone: () => void }) {
  const badge = YOUJI_BADGES.find(b => b.id === badgeId)
  const onDoneRef = useRef(onDone)
  useEffect(() => {
    const t = setTimeout(() => onDoneRef.current(), 2500)
    return () => clearTimeout(t)
  }, [])
  if (!badge) return null
  return (
    <div className={`fixed bottom-24 left-1/2 -translate-x-1/2 z-50 px-5 py-3 rounded-2xl shadow-xl text-center
      ${type === 'gold'
        ? 'bg-gradient-to-r from-yellow-400 to-amber-500 text-white'
        : 'bg-gradient-to-r from-gray-300 to-gray-400 text-gray-800'
      }`}
    >
      <div className="text-2xl">{badge.emoji}</div>
      <div className="font-bold text-sm">{type === 'gold' ? '🥇ゴールドバッジ！' : '🥈シルバーバッジ！'}</div>
      <div className="text-xs mt-0.5">{badge.name}を　ゲット！</div>
      <div className="flex justify-center mt-1 gap-0.5">
        {[...Array(5)].map((_, i) => (
          <span key={i} className="text-xs animate-pulse" style={{ animationDelay: `${i * 0.1}s` }}>✨</span>
        ))}
      </div>
    </div>
  )
}

// ─── 回答後レビュー画面 ──────────────────────────────────
function ReviewScreen({
  question,
  selectedIndex,
  onNext,
  consecutiveCorrect,
}: {
  question: (typeof YOUJI_QUESTIONS)[0]
  selectedIndex: number
  onNext: () => void
  consecutiveCorrect: number
}) {
  const correct = selectedIndex === question.correctIndex
  const streakMsg =
    consecutiveCorrect >= 10 ? '🏆 10もん　れんぞくせいかい！すごすぎ！！'
    : consecutiveCorrect >= 5  ? '🎆 5もん　れんぞく！やったね！'
    : consecutiveCorrect >= 3  ? '✨ 3もん　れんぞく！ナイス！'
    : null

  const rabbitMood: RabbitMood = consecutiveCorrect >= 5 ? 'dancing'
    : consecutiveCorrect >= 3 ? 'excited'
    : correct ? 'happy'
    : 'cheer'

  return (
    <div className="flex flex-col h-full">
      <div className={`px-4 py-4 text-center ${correct ? 'bg-green-50' : 'bg-red-50'}`}>
        <div className={`text-2xl font-bold mb-1 ${correct ? 'text-green-600' : 'text-red-500'}`}>
          {correct ? '✅ せいかい！' : 'おしい！'}
        </div>
        <RabbitCharacter mood={rabbitMood} />
        {streakMsg && (
          <div className="mt-2 text-sm font-bold text-purple-600 animate-pulse">{streakMsg}</div>
        )}
      </div>

      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <div className="text-xs text-gray-400 font-medium mb-2">📌 もんだいの　ふりかえり</div>
          <div className="text-base text-gray-800 font-medium leading-relaxed">{question.question}</div>
        </div>

        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-2">
          <div className="text-xs text-gray-400 font-medium mb-2">こたえの　かくにん</div>
          <div className={`flex items-center gap-2 p-2 rounded-xl text-sm
            ${correct ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'}`}
          >
            <span>{correct ? '✅' : '❌'}</span>
            <span className="font-medium">あなたのこたえ：</span>
            <span>{question.options[selectedIndex]}</span>
          </div>
          {!correct && (
            <div className="flex items-center gap-2 p-2 rounded-xl text-sm bg-green-50 text-green-700">
              <span>⭕</span>
              <span className="font-medium">ただしいこたえ：</span>
              <span>{question.options[question.correctIndex]}</span>
            </div>
          )}
        </div>

        <div className="bg-pink-50 rounded-2xl p-4 shadow-sm border border-pink-100">
          <div className="text-pink-600 font-bold text-sm mb-2">💡 なぜそうなるの？</div>
          <div className="text-sm text-gray-700 leading-relaxed">
            {correct ? question.feedback.correct : question.feedback.incorrect}
          </div>
        </div>

        <div className="pt-2 pb-6">
          <button
            onClick={onNext}
            className="w-full bg-pink-500 hover:bg-pink-600 active:scale-95 text-white font-bold py-4 rounded-2xl text-base transition-all shadow-md"
          >
            わかった！　つぎへ →
          </button>
        </div>
      </div>
    </div>
  )
}

// ─── レベル結果画面 ───────────────────────────────────────
function LevelResultScreen({
  level,
  score,
  progress,
  newSilver,
  newGold,
  onRetry,
  onNext,
}: {
  level: number
  score: number
  progress: YoujiProgress
  newSilver: string[]
  newGold: string[]
  onRetry: () => void
  onNext: () => void
}) {
  const cleared = score >= YOUJI_UNLOCK_THRESHOLD
  const stars = getYoujiLevelStars(score)
  const nextUnlocked = isYoujiLevelUnlocked(level + 1, progress)

  return (
    <div className="flex flex-col items-center justify-center h-full px-6 text-center space-y-6">
      <div className="text-5xl">
        {score >= YOUJI_QUESTIONS_PER_LEVEL ? '🏆' : cleared ? '🎉' : '😿'}
      </div>

      <div>
        <div className={`text-2xl font-bold ${cleared ? 'text-green-600' : 'text-gray-500'}`}>
          {score}/{YOUJI_QUESTIONS_PER_LEVEL}もん　せいかい！
        </div>
        <div className="text-3xl mt-2">
          {[...Array(3)].map((_, i) => (
            <span key={i} className={i < stars ? 'text-yellow-400' : 'text-gray-200'}>⭐</span>
          ))}
        </div>
      </div>

      {score >= YOUJI_QUESTIONS_PER_LEVEL && (
        <div className="bg-gradient-to-r from-yellow-400 to-amber-500 rounded-2xl px-6 py-3 w-full shadow-md">
          <div className="text-white font-bold text-lg">🎊 パーフェクト！</div>
          <div className="text-yellow-100 text-sm">5もん　ぜんぶ　せいかい！すごい！！</div>
        </div>
      )}

      {cleared ? (
        <div className="bg-green-50 border border-green-200 rounded-2xl px-6 py-3">
          <div className="text-green-700 font-bold">
            {level < YOUJI_TOTAL_LEVELS && nextUnlocked ? `🔓 レベル${level + 1}が　ひらいたよ！` : 'クリア！'}
          </div>
        </div>
      ) : (
        <div className="bg-orange-50 border border-orange-200 rounded-2xl px-5 py-3">
          <div className="text-orange-700 font-bold text-sm">
            あと{YOUJI_UNLOCK_THRESHOLD - score}もん　せいかいで　つぎのレベルへ！
          </div>
        </div>
      )}

      {(newSilver.length > 0 || newGold.length > 0) && (
        <div className="bg-yellow-50 rounded-2xl p-3 w-full">
          <div className="text-sm font-bold text-yellow-700 mb-1">🏅 バッジ　ゲット！</div>
          <div className="flex flex-wrap gap-2 justify-center">
            {newGold.map(id => {
              const b = YOUJI_BADGES.find(x => x.id === id)
              return b ? (
                <span key={id} className="text-xs bg-yellow-200 text-yellow-800 px-2 py-1 rounded-full">
                  🥇{b.name}
                </span>
              ) : null
            })}
            {newSilver.map(id => {
              const b = YOUJI_BADGES.find(x => x.id === id)
              return b ? (
                <span key={id} className="text-xs bg-gray-200 text-gray-700 px-2 py-1 rounded-full">
                  🥈{b.name}
                </span>
              ) : null
            })}
          </div>
        </div>
      )}

      <div className="flex flex-col gap-3 w-full">
        <button
          onClick={onRetry}
          className="w-full bg-white border-2 border-pink-300 text-pink-600 font-bold py-3 rounded-2xl active:scale-95 transition-all"
        >
          ← もう　いちど　チャレンジ
        </button>
        {cleared && level < YOUJI_TOTAL_LEVELS && (
          <button
            onClick={onNext}
            className="w-full bg-pink-500 text-white font-bold py-4 rounded-2xl shadow-md active:scale-95 transition-all"
          >
            つぎのレベルへ →
          </button>
        )}
        <Link
          href="/apps/thinking-youji"
          className="w-full text-center text-gray-400 text-sm py-2"
        >
          レベルいちらんに　もどる
        </Link>
      </div>
    </div>
  )
}

// ─── メインゲーム画面 ─────────────────────────────────────
type Phase = 'question' | 'review' | 'result'

export default function YoujiGamePage() {
  const params = useParams()
  const router = useRouter()
  const level = Number(params.level)

  const [progress, setProgress] = useState<YoujiProgress | null>(null)
  const [phase, setPhase] = useState<Phase>('question')
  const [questions, setQuestions] = useState<typeof YOUJI_QUESTIONS>([])
  const [current, setCurrent] = useState(0)
  const [selected, setSelected] = useState<number | null>(null)
  const [correctIds, setCorrectIds] = useState<number[]>([])
  const [showFireworks, setShowFireworks] = useState(false)
  const [toasts, setToasts] = useState<{ id: string; type: 'silver' | 'gold' }[]>([])
  const [resultData, setResultData] = useState<{ newSilver: string[]; newGold: string[] }>({ newSilver: [], newGold: [] })

  useEffect(() => {
    const p = loadYoujiProgress()
    setProgress(p)
    if (getUserType() === 'guest' && level > GUEST_FREE_LEVELS) {
      router.replace('/register')
      return
    }
    if (!isYoujiLevelUnlocked(level, p)) {
      router.replace('/apps/thinking-youji')
      return
    }
    const qs = YOUJI_QUESTIONS.filter(q => q.level === level)
    setQuestions(qs)
  }, [level, router])

  if (!progress || questions.length === 0) return null

  const q = questions[current]
  const consecutiveCorrect = progress.consecutiveCorrect

  function handleAnswer(idx: number) {
    if (selected !== null || !progress) return
    setSelected(idx)

    const correct = idx === q.correctIndex
    const newCorrectIds = correct ? [...correctIds, q.id] : correctIds
    setCorrectIds(newCorrectIds)

    const updatedProgress = updateYoujiStreak(correct, progress)
    setProgress(updatedProgress)
    saveYoujiProgress(updatedProgress)

    if (updatedProgress.consecutiveCorrect >= 5) {
      setShowFireworks(true)
      setTimeout(() => setShowFireworks(false), 1500)
    }

    setPhase('review')
  }

  function handleNext() {
    if (current + 1 < questions.length) {
      setCurrent(c => c + 1)
      setSelected(null)
      setPhase('question')
    } else {
      if (!progress) return
      const { updated, newSilver, newGold } = recordYoujiLevelResult(level, correctIds, progress)
      setProgress(updated)
      saveYoujiProgress(updated)
      setResultData({ newSilver, newGold })

      const allToasts = [
        ...newGold.map(id => ({ id, type: 'gold' as const })),
        ...newSilver.filter(id => !newGold.includes(id)).map(id => ({ id, type: 'silver' as const })),
      ]
      if (allToasts.length > 0) setToasts(allToasts)

      if (correctIds.length >= YOUJI_UNLOCK_THRESHOLD) {
        setShowFireworks(true)
        setTimeout(() => setShowFireworks(false), 2000)
      }

      setPhase('result')
    }
  }

  function handleRetry() {
    const p = loadYoujiProgress()
    setProgress(p)
    setCurrent(0)
    setSelected(null)
    setCorrectIds([])
    setPhase('question')
  }

  function handleNextLevel() {
    router.push(`/apps/thinking-youji/${level + 1}`)
  }

  const score = correctIds.length

  return (
    <div className="flex flex-col h-screen bg-gradient-to-b from-pink-50 to-orange-50">
      <Fireworks active={showFireworks} />

      {toasts.length > 0 && (
        <BadgeToast
          badgeId={toasts[0].id}
          type={toasts[0].type}
          onDone={() => setToasts(t => t.slice(1))}
        />
      )}

      {phase !== 'result' && (
        <div className="bg-white shadow-sm px-4 py-3 flex items-center gap-3 shrink-0">
          <Link href="/apps/thinking-youji" className="text-gray-400 text-lg">←</Link>
          <div className="flex-1">
            <div className="text-sm font-bold text-gray-700">🐰 レベル {level}</div>
            <div className="text-xs text-gray-400">
              {YOUJI_UNLOCK_THRESHOLD}もん　せいかいで　クリア
            </div>
          </div>
          <div className="text-right">
            <div className="text-xs text-gray-400 mb-1">
              ✓ <span className="font-bold text-green-600">{score}</span> / {YOUJI_QUESTIONS_PER_LEVEL}もん　せいかい
            </div>
            <div className="flex gap-1">
              {questions.map((_, i) => {
                const answered = i < current || (phase === 'review' && i === current)
                const isCorrect = answered && correctIds.includes(questions[i].id)
                return (
                  <div
                    key={i}
                    className={`w-4 h-2 rounded-full transition-all
                      ${answered ? (isCorrect ? 'bg-green-400' : 'bg-red-300')
                        : i === current ? 'bg-pink-400'
                        : 'bg-gray-200'}`}
                  />
                )
              })}
            </div>
          </div>
        </div>
      )}

      <div className="flex-1 overflow-hidden">
        {phase === 'question' && (
          <QuestionScreen
            question={q}
            questionIndex={current}
            total={questions.length}
            onAnswer={handleAnswer}
          />
        )}
        {phase === 'review' && selected !== null && (
          <ReviewScreen
            question={q}
            selectedIndex={selected}
            onNext={handleNext}
            consecutiveCorrect={consecutiveCorrect}
          />
        )}
        {phase === 'result' && progress && (
          <LevelResultScreen
            level={level}
            score={score}
            progress={progress}
            newSilver={resultData.newSilver}
            newGold={resultData.newGold}
            onRetry={handleRetry}
            onNext={handleNextLevel}
          />
        )}
      </div>
    </div>
  )
}

// ─── 問題画面 ─────────────────────────────────────────────
function QuestionScreen({
  question, questionIndex, total, onAnswer,
}: {
  question: (typeof YOUJI_QUESTIONS)[0]
  questionIndex: number
  total: number
  onAnswer: (idx: number) => void
}) {
  return (
    <div className="flex flex-col h-full">
      <div className="px-4 pt-3 pb-2 flex items-center justify-between shrink-0">
        <span className="text-sm font-bold text-gray-500">{questionIndex + 1} / {total}</span>
      </div>

      <div className="px-4 pb-3 flex-1 flex flex-col gap-4 overflow-y-auto">
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          <div className="text-lg leading-relaxed text-gray-800 font-bold text-center">
            {question.question}
          </div>
        </div>

        <div className="space-y-3 pb-4">
          {question.options.map((opt, i) => (
            <button
              key={i}
              onClick={() => onAnswer(i)}
              className="w-full text-left bg-white border-2 border-gray-200 rounded-2xl px-4 py-4
                text-base text-gray-700 font-medium
                active:scale-95 hover:border-pink-300 hover:bg-pink-50
                transition-all duration-150 shadow-sm"
            >
              <span className="inline-block w-7 h-7 rounded-full bg-pink-100 text-pink-600 text-sm font-bold text-center leading-7 mr-3 shrink-0">
                {['①', '②', '③', '④'][i]}
              </span>
              {opt}
            </button>
          ))}
        </div>
      </div>
    </div>
  )
}
