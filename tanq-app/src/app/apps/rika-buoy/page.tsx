...
// 中学受験 理科〈浮力〉— 計算・思考分野の単元（rika-chukan のひな型を踏襲）
// 方式: まなぶ（図解導入スライド）→ とく（演習）。
// 演習は2段階ヒント: 1回目不正解=考える足場 → 2回目不正解=答え＋図解説明。
// スコア・進捗は初回解答のみで記録（リトライで水増ししない）。
// 答えバレ防止: 求める量（浮力・ばねばかりの読み・水面上昇など）は解答前に図へ実数を出さず「?」表示。
//   reveal 後のみ実数を表示する。

import React, { useState, useEffect, useCallback } from 'react'
import Link from 'next/link'
import { BUOY_SLIDES, BUOY_PROBLEMS, BuoyProblem, BuoySlide, BuoyFigure, BuoyDifficulty } from '@/data/rikaBuoyData'
...
const TEAL = '#2BA39A'
const WATER = '#7FB8F0'
const PROGRESS_KEY = 'tanq_rika_buoy_progress_v1'
...
const DIFF_LABEL: Record<BuoyDifficulty, { label: string; color: string; bg: string }> = {
...
// ─────────────────────────────────────
// SVG: ビーカー＋水＋物体＋（任意で）ばねばかり
//  求める量は解答前に図へ実数を出さず「?」（answer バレ防止）。
//  state: sink=底に沈む / submerged=水中につるす / float=水面に浮く
// ─────────────────────────────────────
function BuoyDiagram({ fig, reveal }: { fig: BuoyFigure; reveal: boolean }) {
  const W = 130, H = 150
  const bx = 30, bw = 70, btop = 44, bbot = H - 14
  const waterTop = btop + 14
  const objW = 34
  const objCx = bx + bw / 2

  // 物体の縦位置（state で変わる）
  let objTop: number, objH: number
  let aboveWater = 0   // 水面より上に出ている高さ（float のみ）
  if (fig.state === 'float') {
    objH = 40
    const ratio = fig.aboveRatio == null ? 0.4 : fig.aboveRatio
    aboveWater = objH * ratio
    objTop = waterTop - aboveWater
  } else if (fig.state === 'submerged') {
    objH = 36
    objTop = waterTop + 22
  } else { // sink
    objH = 32
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-[150px] mx-auto overflow-visible">
      {/* ばねばかり（submerged で hasSpring のとき・上にぶら下げる） */}
      {fig.hasSpring && (
        <>
          <line x1={objCx} y1={6} x2={objCx} y2={objTop} stroke={INK} strokeWidth="1.6" strokeDasharray="2 2" />
          <rect x={objCx - 14} y={4} width={28} height={14} rx="3" fill="#FFF" stroke={INK} strokeWidth="1.6" />
          <text x={objCx} y={14} textAnchor="middle" fontSize="8" fill={INK} fontWeight="bold">
            {valLabel(fig.springRead, 'g', reveal)}
          </text>
        </>
      )}

      {/* 水 */}
      <rect x={bx + 2} y={waterTop} width={bw - 4} height={bbot - waterTop - 1} fill={WATER} opacity="0.5" />
      {/* 水面ライン */}
      <line x1={bx + 2} y1={waterTop} x2={bx + bw - 2} y2={waterTop} stroke={WATER} strokeWidth="2" />

      {/* 物体 */}
      <rect x={objCx - objW / 2} y={objTop} width={objW} height={objH} rx="3"
        fill={fig.state === 'float' ? '#D9B98A' : '#9AA0A6'} stroke={INK} strokeWidth="2" />
      {/* float のときは水面から上の部分を明るく（空気中） */}
      {fig.state === 'float' && aboveWater > 0 && (
        <rect x={objCx - objW / 2} y={objTop} width={objW} height={aboveWater} fill="#F0DCBE" stroke="none" />
      )}
      {fig.state === 'float' && (
        <rect x={objCx - objW / 2} y={objTop} width={objW} height={objH} rx="3" fill="none" stroke={INK} strokeWidth="2" />
      )}
      ...
      {/* 物体の重さ・浮力の注記（求める量は ? 表示） */}
      {(fig.weight !== undefined) && (
        <text x={objCx} y={objTop + objH / 2 + 3} textAnchor="middle" fontSize="7.5" fill={INK} fontWeight="bold">
          {valLabel(fig.weight, 'g', reveal)}
        </text>
      )}
      {(fig.buoyancy !== undefined) && (
        <>
          {/* 上向きの浮力矢印 */}
          <line x1={objCx + objW / 2 + 8} y1={objTop + objH} x2={objCx + objW / 2 + 8} y2={objTop + 4}
            stroke="#E06A2C" strokeWidth="2" markerEnd="url(#buoyArrow)" />
          <defs>
            <marker id="buoyArrow" markerWidth="6" markerHeight="6" refX="3" refY="3" orient="auto">
              <path d="M0,0 L6,3 L0,6 Z" fill="#E06A2C" />
            </marker>
          </defs>
          <text x={objCx + objW / 2 + 11} y={objTop + objH / 2} fontSize="7.5" fill="#E06A2C" fontWeight="bold">
            {valLabel(fig.buoyancy, 'g', reveal)}
          </text>
        </>
      )}
      ...
      {/* ラベル */}
      {fig.label && (
        <text x={W / 2} y={btop - 8} textAnchor="middle" fontSize="9" fill={TEAL} fontWeight="bold">{fig.label}</text>
      )}
    </svg>
  )
}
...
// 1つの図ユニット
function FigureUnit({ fig, reveal }: { fig: BuoyFigure; reveal: boolean }) {
  return (
    <div className="flex flex-col items-center">
      <BuoyDiagram fig={fig} reveal={reveal} />
...
// 複数の図を並べる
function BuoyFigures({ figs, reveal = false }: { figs: BuoyFigure[]; reveal?: boolean }) {
...
// まなぶ: スライド
// ─────────────────────────────────────
function SlideCard({ slide }: { slide: BuoySlide }) {
  return (
    <div className="rounded-[22px] overflow-hidden" style={{ border: `3px solid ${INK}`, boxShadow: `4px 4px 0 0 ${INK}` }}>
      ...
      <div className="px-4 pt-4 pb-2" style={{ background: '#FFFFFF' }}>
        {slide.diagrams && <BuoyFigures figs={slide.diagrams} reveal />}
      </div>
    ...
    <div className="space-y-4">
      ...
      {!finished && problem.diagrams.length > 0 && (
        <div className="rounded-[18px] p-3" style={{ background: '#FFFFFF', border: `2.5px solid ${INK}`, boxShadow: `3px 3px 0 0 ${INK}` }}>
          <BuoyFigures figs={problem.diagrams} />
        </div>
      )}
      ...
      {status === 'correct' && (
        <div className="rounded-[18px] p-4 space-y-3" style={{ background: '#DBF6F0', border: `3px solid ${INK}`, boxShadow: `4px 4px 0 0 ${INK}` }}>
          ...
          <div className="rounded-2xl bg-white/70 p-2">
            <BuoyFigures figs={problem.diagrams} reveal />
          </div>
...
// メインページ
// ─────────────────────────────────────
export default function RikaBuoyPage() {
  const [phase, setPhase] = useState<Phase>('manabu')
  const [slideIdx, setSlideIdx] = useState(0)
  const [selectedDiff, setSelectedDiff] = useState<BuoyDifficulty>(1)
  ...
  const problemsOf = useCallback((d: BuoyDifficulty) => BUOY_PROBLEMS.filter(p => p.difficulty === d), [])
  ...
  const goNext = useCallback(() => {
    if (problemIndex < currentList.length - 1) {
      ...
      if (attempted.length > 0) {
        ...
        saveScore('rika-buoy', score, attempted.length, `★${selectedDiff}`)
      }
      setSessionResults({})
      setProblemIndex(0)
      setPhase('list')
  ...
  const totalSolved = solvedIds.filter(id => BUOY_PROBLEMS.some(p => p.id === id)).length
  const totalProblems = BUOY_PROBLEMS.length
  const pct = Math.round(totalSolved / totalProblems * 100)
  const slide = BUOY_SLIDES[slideIdx]
            ...
            <>
              <div className="flex items-center justify-between">
                <span className="text-xs font-black px-2.5 py-1 rounded-full"
                  ...
                  📖 まなぶ {slideIdx + 1}/{BUOY_SLIDES.length}
                </span>
