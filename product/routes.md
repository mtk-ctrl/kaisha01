...
**lab での権限**: `tanq` 以外のすべてのアプリにアクセス可（science・kokugo を含む）
...
- ※ science・kokugo・english・clock・shapes・coding は **member または tester のみ**
...
function canAccessApp(appId: string, userType: UserType): boolean {
  ...
  if (userType === 'member') return appId !== 'tanq'             // ①② tanq以外（science・kokugo含む）
  // guest: 体験用アプリのみ                                       // ③
  return ['math','kanji','word-math','kuku','todofuken',
          'thinking','thinking-youji'].includes(appId)
         || appId.startsWith('youji-')
  // science・kokugo・english・clock・shapes・coding は member/tester のみ
}
```

> **更新**: 2026-05-25 — science・kokugo を member/tester 限定に明記
