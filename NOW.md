...
> **最終更新**: 2026-07-11（オーナー指摘②件を反映: **①ステップ4を「文章えらび」方式に改修**＝1文章1セット4問・★難度表示（複数文章が8問連続で流れる設計ミスを解消）**②難度ラダーを明示**＝★導入→★★ふつう→**★★★入試レベル1200〜1600字（3本目以降の標準）**。徐々に長く・難しくする） | 更新者: Jobs
...
1. **国語読解の本格化（優先1・進行中）** — ステップ4〈長文読解〉2本公開済み（s4-01「さいごのごめんね」★／s4-02「里山」★★）・**文章えらび方式**（1文章=1セット4問・★難度表示）。**オーナー判定「まだ短い・簡単」→3本目以降は ★★★＝入試レベルを標準にする: 1200〜1600字・設問5〜6問・心情の変化/対比/言いかえ/記述的な選択肢**。次は3本目（物語★★★）を1本だけ作る。AI生成オリジナル文（転載禁止は既決）・紛らわしい選択肢3類型（問いに合わない/言い過ぎ/本文にない）・根拠文ハイライト・`paraBreaks` で段落。**細切れ原則＝1本仕上げ→画面確認→オーナー判定→次へ**。「ためしてみる版」表記は長文6本そろうまで維持（アキ判断）。記述＋AI採点はスコープ外（後続）
...
| 日付 | 内容 |
|------|------|
| 2026-07-11 | ステップ4を「文章えらび」方式に改修（オーナー指摘=別文章が1つにまとまり8問連続で流れる設計ミス）: 1文章=1セット4問・title/level(★〜★★★)を DokkaiPassage に追加・文章えらび画面新設・結果画面も文章単位（もう一度この文章/つぎの文章/文章えらびへ）・saveScoreラベルも「長文『題名』」に。難度ラダー=★導入→★★→★★★入試レベル(1200〜1600字・3本目以降の標準)を明示 |
| 2026-07-11 | 読解ステップ4 2本目「里山」公開（s4-02・説明文約870字×4問=接続語/指示語/理由/要点・難度を中受寄りに引き上げ・読解計54問・Playwright 3画面目視・build EXIT=0） |
