...
> **最終更新**: 2026-07-11（**理想状態（North Star 7条件）とロードマップ4ステージを制定**＝`company/roadmap.md` 新設。以後、全案件は「7条件のどれを何%進めるか」で管理。同日: コンテンツ拡張の優先順位も決定（優先1 国語読解本格化・優先2 演習量3層化＝Stage 1）。今回は仕様議論のみ・実装は次セッションから③の順で着手） | 更新者: Jobs
