...
> **最終更新**: 2026-07-11（**読解ステップ4 2本目「里山」公開**＝説明文・約870字×4問・難度を中受寄りに引き上げ（譲歩→逆接の接続語・筆者の主張・「本文にあるが主張ではない」選択肢）。読解は計54問。次はオーナー判定→OKなら3本目） | 更新者: Jobs
...
1. **国語読解の本格化（優先1・進行中）** — ステップ4〈長文読解〉2本公開済み: **1本目 s4-01「さいごのごめんね」（物語約890字・導入なのでやさしめ）／2本目 s4-02「里山」（説明文約870字・中受寄り＝譲歩→逆接・筆者の主張・原生林/常緑樹/矛盾などの語彙）**。2本目のオーナー判定待ち→OKなら3本目（物語・中受寄り難度）を1本だけ作る。AI生成オリジナル文（転載禁止は既決）・紛らわしい選択肢3類型（問いに合わない/言い過ぎ/本文にない）・根拠文ハイライト・`paraBreaks` で段落。**細切れ原則＝1本仕上げ→画面確認→オーナー判定→次へ**。「ためしてみる版」表記は長文6本そろうまで維持（アキ判断）。記述＋AI採点はスコープ外（後続）
