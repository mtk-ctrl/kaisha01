...
**今すぐやること（優先順）**:
1. **都道府県アプリ 本番確認** — Vercelデプロイ後：オーナーが `/youji/apps/todofuken/` を開いて形・disabled・地図を確認
2. **ようちえん かんがえるジム 本番確認** — `/apps/thinking-youji` でゲスト体験（Lv1-2）確認
3. **テスター招待フローの確認** — `/tester` ページのURLを招待する人にシェアする
...
| 日付 | 内容 | ログ |
|------|------|------|
| 2026-05-23 | 都道府県アプリ 全面修正（バグ7件修正・GeoJSON実形状データ刷新・Playwright画面レビュー導入・ルール改定） | コミット履歴参照 |
