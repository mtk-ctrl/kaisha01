...
> **最終更新**: 2026-07-11（**ロードマップをオーナー承認 → Stage 1 着手**＝読解ステップ4〈長文読解〉を新設し1本目「さいごのごめんね」（物語・約890字・4問・段落表示・紛らわし選択肢3類型）を公開。読解は計50問に。細切れ原則により2本目以降は1本ずつ・オーナー判定を受けてから） | 更新者: Jobs
...
1. **国語読解の本格化（優先1・進行中）** — ステップ4〈長文読解〉新設済み・**1本目「さいごのごめんね」公開済み（2026-07-11・s4-01・物語約890字×4問）**。次は2本目=説明文（800〜1200字×4問）を1本だけ作る。AI生成オリジナル文（転載禁止は既決）・紛らわしい選択肢3類型（問いに合わない/言い過ぎ/本文にない）・根拠文ハイライト・`paraBreaks` で段落。**細切れ原則＝1本仕上げ→画面確認→オーナー判定→次へ**。「ためしてみる版」表記は長文6本そろうまで維持（アキ判断）。記述＋AI採点はスコープ外（後続）
